...
def get_args():
    ...
    # --- v4 additions: data setup + seed-aware output ---
    p.add_argument('--no_seed_subdir', action='store_true',
                   help='Use the legacy outputs_hamseg/{dataset}/ layout instead of '
                        'outputs_hamseg/{dataset}/seed_{seed}/ (back-compat only).')
    p.add_argument('--skip_data_check', action='store_true',
                   help='Skip the dataset existence check at startup.')
    ...
    # Auto dataset subfolder: outputs_hamseg/{dataset}/[abl_X/]seed_{seed}/  (v4 default)
    a.output_dir = os.path.join(a.output_dir, a.dataset)
    if a.ablation != 'none':
        a.output_dir = os.path.join(a.output_dir, f'abl_{a.ablation}')
...
class HamiltonianBottleneck(nn.Module):
    ...
    ablation='none' (default) — full bottleneck: ConvNeXt + SS2D oscillator
        with sigmoid-gated fusion; emits (out, momentum, energy_map).

    ablation='A' (ConvNeXt-only) — drops the entire SS2D oscillator path. The
        bottleneck becomes two stacked ConvNeXt blocks at the same channel dim.
        momentum and energy_map are returned as None so the decoder skip
        connections degrade gracefully to plain U-Net skips.

    ablation='B' (Oscillator-only) — drops the ConvNeXt path and the gate.
    ...
    def __init__(self, dim, damping_clamp=5.0, drop_rate=0.1,
        ...
        if ablation == 'A':
            ...
            # full HamSeg's conv_block depth exactly. The only difference between
            # variant A and full is that variant A has no SS2D / gate / energy_attn
            # apparatus. This is the clean "is the Hamiltonian apparatus essential"
            # ablation; the alternative "double the conv depth to compensate for
            # the missing SS2D path" inflates capacity and biases the comparison
            # in variant A's favour.
        ...
        # Default (ablation='none'): full bottleneck with ConvNeXt + gated fusion
    ...
    def forward(self, x):
        # ----- Variant A: ConvNeXt-only (no oscillator) -----
        if self.ablation == 'A':
        ...
        # then enter the fp32 oscillator block; closure captures conv_out fine.
        if self.ablation != 'B':
    ...
    def forward(self, dec_feat, enc_feat, energy=None, momentum=None):
        ...
        if self.use_momentum:
            if momentum is not None:
                parts.append(self.mom_proj(momentum))
            else:
                # Ablation A: bottleneck did not produce momentum. Append
                # a zero tensor so the reduce conv's channel count is unchanged
                # (3*dec_ch). The mom_proj weights remain in the model state
                # but contribute nothing this forward.
                parts.append(torch.zeros_like(dec_feat))
...
class HamSeg(nn.Module):
    ...
    def forward(self, x):
        ...
        # When ablation='A', momentum and energy are None — the decoder
        # gracefully falls back to plain U-Net skips (EnergyGatedSkip already
        # handles None for both, and we skip the PhaseSpaceAttention call).
        no_phys = momentum is None or energy is None

        d3 = self.up3(e4)
        if no_phys:
            d3 = self.skip3(d3, e3, None, None)        # plain skip
            d3 = self.dec3(d3)
        else:
...
def main():
    args = get_args()
    set_seed(args.seed)
    # output_dir is now: outputs_hamseg/{dataset}/seed_{seed}/  (legacy: --no_seed_subdir)
    logger = setup_logging(args.output_dir)
    logger.info('='*60)
    logger.info(f'  HamSeg v4: CNN backbone + Hamiltonian innovations')
    logger.info(f'  Dataset: {args.dataset}    Seed: {args.seed}')
    logger.info(f'  Output:  {args.output_dir}')
    logger.info('='*60)
    for k in ['device','dataset','img_size','num_classes','embed_dim','depths',
              'epochs','batch_size','lr','weight_decay','drop_rate','use_amp','seed']:
        logger.info(f'  {k:15s}: {getattr(args, k)}')

    # --- v4 robust result preservation: save args.json explicitly ---
    args_dict = {k: (str(v) if hasattr(v, '__class__') and not isinstance(
                       v, (int, float, str, bool, list, type(None))) else v)
                 for k, v in vars(args).items()}
    with open(os.path.join(args.output_dir, 'args.json'), 'w') as f:
        json.dump(args_dict, f, indent=2)
    logger.info(f'Saved launch args to {args.output_dir}/args.json')

    # --- v4 dataset check + auto-download ---
    if not args.skip_data_check:
        try:
            from data_setup import prepare
            ok = prepare(args.dataset, args.data_root,
                         auto_download=not args.no_auto_download)
            if not ok:
                logger.error(f'Dataset {args.dataset} is not ready at {args.data_root}.')
                logger.error('Re-run with --skip_data_check to bypass this guard.')
                sys.exit(2)
        except ImportError:
            logger.warning('data_setup.py not found; skipping dataset existence check.')
    ...
    train_ds = MedicalSegDataset(args.data_root, 'train', args.img_size,
                                 args.train_ratio,
                               args.val_ratio, args.num_classes)
    ...
    model_info = {
        ...
        'params_total_mb_fp32': total * 4 / 1024 ** 2,
        'embed_dim': args.embed_dim,
        'depths': list(args.depths),
        'damping_clamp': args.damping_clamp,
        'drop_rate': args.drop_rate,
        'img_size': args.img_size,
        'num_classes': args.num_classes,
    }
    # Add FLOPs if fvcore is installed (graceful fallback otherwise).
    try:
        ...
        model_info['flops_total'] = int(flops.total())
        model_info['gflops_at_input'] = float(flops.total() / 1e9)
        logger.info(f'  GFLOPs (224x224): {model_info["gflops_at_input"]:.2f}')
    except Exception as e:
        model_info['flops_total'] = None
        logger.info(f'  (fvcore not available, skipping FLOPs: {e})')
